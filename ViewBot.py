...
def ReadProxiesFile():
    ...
    while restartTry:
        try:
            ...
            Path = Write.Input("Proxies > ", Colors.red_to_purple, interval=0.0001)
            restartTry = True
...
if (__name__ == "__main__"):
    ...
    amount       = Write.Input("Anzahl (0=inf) > ", Colors.red_to_purple, interval=0.0001)
    ScrapProxie  = Write.Input("Scrap Proxies [y/n] > ", Colors.red_to_purple, interval=0.0001)
    Proxytype    = Write.Input("Proxy Typ > ", Colors.red_to_purple, interval=0.0001)
    Timeout      = Write.Input("Proxy Timeout > ", Colors.red_to_purple, interval=0.0001)
    NThread      = Write.Input("VERIFY | Gib irgendeine zahl ein > ", Colors.red_to_purple, interval=0.0001)
    
    if Title("Proxy Scrapper X-Proxy by Jonas Sch.") == True:
        Debug = Write.Input("Debug Fehlgeschlagen [y/n] ? > ", Colors.red_to_purple, interval=0.0001)
    ...
    while ProxyChoose:
        if Proxytype.lower().startswith("h"):
            ...
            ProxyChoose = True
            print("Falscher Proxy Typ | Wähle : Http, Socks4, Socks5")
            Proxytype = Write.Input("Proxy Typ > ", Colors.red_to_purple, interval=0.0001)

    proxy = ReadProxiesFile()
    
    print(Colorate.Horizontal(Colors.red_to_white, f"Keine Anzahl!!! "))
    print(Colorate.Horizontal(Colors.red_to_white, f" Guck in 5 Minuten nochmal!"))
